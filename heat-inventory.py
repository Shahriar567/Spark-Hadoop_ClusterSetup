...
# Command line agrv takes:
# 1st ) name of the stack
# 2nd ) the name of the master node
...
class heat_inventory:
    ...
    # templates
    host_entry = Template('$ipaddress             ansible_connection=ssh  ansible_ssh_user=$ssh_user api_hostname=$master_hosts  ansible_ssh_private_key_file=$private_key_file')
...
$nodes
    ...
    def __init__(self, stack_n, master_n):
        self.stack_name = stack_n
        self.master_name = master_n
        self.load_heat_output()


    def load_heat_output(self):
        #self.heat_output = json.dumps(subprocess.Popen("heat output-show bananicustom --all 2>&1 | grep -v WARN | grep -v 'You can upgrade' | grep -v  'SNI' | grep -v '^192' | grep -v '^206' | grep -v 'Insecure' ", shell=True, stdout=subprocess.PIPE).stdout.read())
        com_pnet = "echo [ && heat output-show "+ self.stack_name+" --all --format json  | sed -e 's/}/},/g' | head -n -1 && echo } && echo ]"
        foo = subprocess.Popen(com_pnet, shell=True, stdout=subprocess.PIPE).stdout.read()
        print(foo)
        self.heat_output = json.loads(foo)
    ...
    # Ansible hosts file
    def get_host_entry(self, ipaddress, hostname):
        # host_entry = Template('$ipaddress             ansible_connection=ssh  ansible_ssh_user=$ssh_user api_hostname=$master_host  ansible_ssh_private_key_file=$private_key_file')
        return self.host_entry.substitute(ipaddress=ipaddress, ssh_user=self.ansible_ssh_user, master_hosts = hostname, private_key_file=self.ansible_ssh_private_key_file)

    def get_datanode_host_entries(self):
        datanode_hosts = []
        for datanode_host in self.get_datanode_public_ips():
            datanode_hosts.append(self.get_host_entry(datanode_host[0], datanode_host[1]))
        return "\n".join(datanode_hosts)

    def get_hosts_output(self):
        master_host = self.get_host_entry(self.get_master_public_ip(), self.master_name)
        datanode_hosts = self.get_datanode_host_entries()
        return dedent(self.hosts_output.substitute(master_host=master_host, data_hosts=datanode_hosts, hadoop_master_name=self.master_name, master_ip=self.get_master_public_ip()))


    # Ansible group_vars nodes
    def get_node_entry(self, hostname, ipaddress):
        return self.node_entry.substitute(hostname=hostname, ipaddress=ipaddress, master_hosts = "")
...
def main():
    ...
    heat_inv = heat_inventory(stack_name, master_name)
